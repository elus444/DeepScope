...
class CriticAgent:
    ...
    def critique(self, query: str, summary: str, citations: list[dict]):
        """
        ...
        Args:
            ...
            citations: Original retrieved chunks, for reference (not user-facing, so no citation instruction needed here)

        Returns:
            dict with critique feedback
        """
        agent_logger.info(f"{self.name}: Starting critique for query='{query}'")
        agent_logger.debug(f"{self.name}: Summary length: {len(summary)} chars, {len(citations)} chunks available")
        ...
        context = build_numbered_context(citations[:3])  # Use top 3 chunks for context
        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction="You are a critical evaluator focused on ensuring comprehensive and accurate answers.",
                    temperature=0.4,
                ),
            )
